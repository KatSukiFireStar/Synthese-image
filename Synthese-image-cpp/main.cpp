...
struct ObjectHierarchy
{
    ...
    ObjectHierarchy(const BoundingBox &bounding_box,
                    const vector<shared_ptr<Sphere> > &spheres) : boundingBox(bounding_box), spheres(spheres),
                                                                  leftTree(), hasLeftTree(false), rightTree(),
                                                                  hasRightTree(false) {}

    ObjectHierarchy() : boundingBox(), spheres(), leftTree(), hasLeftTree(false), rightTree(), hasRightTree(false) {}

    ObjectHierarchy(const BoundingBox &bounding_box, shared_ptr<ObjectHierarchy> leftTree,
                    shared_ptr<ObjectHierarchy> rightTree) : isLeaf(false), boundingBox(bounding_box), spheres(),
                                                             leftTree(std::move(leftTree)), hasLeftTree(true),
                                                             rightTree(std::move(rightTree)), hasRightTree(true) {}
};
...
struct IntersectionSphere
{
    ...
    IntersectionSphere(const bool isIntersection, const float distance, const Point intersection,
                       const Direction normal, const Sphere &sphere) : isIntersection(
                                                                           isIntersection), distance(distance),
    ...
    IntersectionSphere() : isIntersection(false), distance(0), intersection(Point{0, 0, 0}), normal(Direction{0, 0, 0}),
                           sphere(Sphere(0, Point{0, 0, 0}, Color::white(), Diffuse)) {}
};
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int l = 0; l < nb; l++)
            {
                ...
                if (it.isIntersection)
                {
                    ...
                    float lightProba = 1.0f / scene.lights.size();
                    ...
                    //Ancien code de visibility pour une lampe
            ...
            fileOut << static_cast<int>(col.red) << " " << static_cast<int>(col.green) << " " << static_cast<int>(col.
                blue) << endl;
        }
        fileOut << endl;
    }
