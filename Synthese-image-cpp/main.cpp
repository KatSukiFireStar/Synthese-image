...
struct ImagePBM {
    ...
    void saveImage(const string& path) const {
        string fullPath = "../" + path + ".pbm";
        ofstream fileOut;
        fileOut.open(fullPath.c_str());
        fileOut << "P1" << endl << width << " " << height << endl;
...
struct ImagePPM {
    ...
    void saveImage(const string& path) const {
        string fullPath = "../" + path + ".ppm";
        ofstream fileOut;
        fileOut.open(fullPath.c_str());
        fileOut << "P3" << endl << width << " " << height << endl << "255" << endl;
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                fileOut << image[i][j].red << " " << image[i][j].green << " " << image[i][j].blue << endl;
            }
            fileOut << endl;
        }
    }
...
void imageWhite() {
    ImagePBM imageWhite = ImagePBM<float, 200, 200>{};

    for (int i = 0; i < imageWhite.width; i++) {
        for (int j = 0; j < imageWhite.height; j++) {
            imageWhite.image[i][j] = 0;
        }
    }
...
void imagePink() {
    ImagePPM imagePink = ImagePPM<200, 300>{};

    for (int i = 0; i < imagePink.width; i++) {
        for (int j = 0; j < imagePink.height; j++) {
            imagePink.image[i][j] = Color{255,192,203};
        }
    }
